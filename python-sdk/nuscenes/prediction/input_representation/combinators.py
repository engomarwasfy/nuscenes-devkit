...
def add_foreground_to_image(base_image: np.ndarray,
    ...
    if base_image.shape != foreground_image.shape:
        raise ValueError(
            f"base_image and foreground image must have the same shape. Received {base_image.shape} and {foreground_image.shape}"
        )

    if base_image.dtype != "uint8" or foreground_image.dtype != "uint8":
        raise ValueError(
            f"base_image and foreground image must be of type 'uint8'. Received {base_image.dtype} and {foreground_image.dtype}"
        )
